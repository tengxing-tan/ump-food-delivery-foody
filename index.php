...
if (isset($_POST["Login"])) {
    ...
    if ($type === 'Admin') {
        ...
        $login = mysqli_query($con, "SELECT admin_ID, admin_email, admin_password FROM `admin` WHERE admin_email='$email' limit 1");
        ...
        $login = mysqli_query($con, "SELECT rider_name, rider_ID, rider_password FROM rider WHERE rider_email = '$email' AND rider_password='$password' limit 1 ");

        if (mysqli_num_rows($login) > 0) {

            $row = mysqli_fetch_assoc($login);

            $_SESSION["rider"] = $row['rider_ID'];

            header("Location: rider/view/home/mainPage.php");
        } else {
            echo "<script>alert('Login details is incorrect. Please try again.');</script>";
        }
    } else if ($type === 'Restaurant Owner') {

        $login = mysqli_query($con, "SELECT ro_name, ro_ID, ro_password FROM restaurantowner WHERE ro_email = '$email' AND ro_password='$password' limit 1 ");

        if (mysqli_num_rows($login) > 0) {

            $row = mysqli_fetch_assoc($login);

            $_SESSION["roID"] = $row['ro_ID'];

            header("Location: restaurant-owner/");
        } else {
            echo "<script>alert('Login details is incorrect. Please try again.');</script>";
        }
    }
